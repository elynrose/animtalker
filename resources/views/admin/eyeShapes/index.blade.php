...
@can('eye_shape_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.eye-shapes.create') }}">
                ...
                <tbody>
                    @foreach($eyeShapes as $key => $eyeShape)
                        <tr data-entry-id="{{ $eyeShape->id }}">
                            ...
                            <td>
                                @can('eye_shape_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.eye-shapes.show', $eyeShape->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('eye_shape_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.eye-shapes.edit', $eyeShape->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('eye_shape_delete')
                                    <form action="{{ route('admin.eye-shapes.destroy', $eyeShape->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('eye_shape_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.eye-shapes.massDestroy') }}",
