...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clips.update", [$clip->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('script') ? 'is-invalid' : '' }}" name="script" id="script" required>{{ old('script', $clip->script) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('i_own_music') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="i_own_music" id="i_own_music" value="1" {{ $clip->i_own_music || old('i_own_music', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}" type="number" name="cost" id="cost" value="{{ old('cost', $clip->cost) }}" step="1">
                @if($errors->has('cost'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cost') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Clip::PRIVACY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('privacy') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="privacy_{{ $key }}" name="privacy" value="{{ $key }}" {{ old('privacy', $clip->privacy) === (string) $key ? 'checked' : '' }}>
