...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.skin-tones.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="tone">{{ trans('cruds.skinTone.fields.tone') }}</label>
                <input class="form-control {{ $errors->has('tone') ? 'is-invalid' : '' }}" type="text" name="tone" id="tone" value="{{ old('tone', '') }}">
                @if($errors->has('tone'))
