    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.backgrounds.update", [$background->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="background_title">{{ trans('cruds.background.fields.background_title') }}</label>
                            <input class="form-control" type="text" name="background_title" id="background_title" value="{{ old('background_title', $background->background_title) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="scene" id="scene" required>{{ old('scene', $background->scene) }}</textarea>
                            @if($errors->has('scene'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('scene') }}
                                </div>
...
<script>
    ...
    url: '{{ route('frontend.backgrounds.storeMedia') }}',
    maxFilesize: 2, // MB
