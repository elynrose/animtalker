        ...
        <div class="col-md-12">
            @can('background_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.backgrounds.create') }}">
                            ...
                            <tbody>
                                @foreach($backgrounds as $key => $background)
                                    <tr data-entry-id="{{ $background->id }}">
                                        ...
                                        <td>
                                            @can('background_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.backgrounds.show', $background->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('background_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.backgrounds.edit', $background->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('background_delete')
                                                <form action="{{ route('frontend.backgrounds.destroy', $background->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('background_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.backgrounds.massDestroy') }}",
