        ...
        <div class="col-md-12">
            @can('dress_color_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.dress-colors.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $dressColor->id }}">
                                        ...
                                        <td>
                                            @can('dress_color_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.dress-colors.show', $dressColor->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('dress_color_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.dress-colors.edit', $dressColor->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('dress_color_delete')
                                                <form action="{{ route('frontend.dress-colors.destroy', $dressColor->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('dress_color_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.dress-colors.massDestroy') }}",
