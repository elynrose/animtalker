        ...
        <div class="col-md-12">
            @can('facial_expression_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.facial-expressions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $facialExpression->id }}">
                                        ...
                                        <td>
                                            @can('facial_expression_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.facial-expressions.show', $facialExpression->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('facial_expression_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.facial-expressions.edit', $facialExpression->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('facial_expression_delete')
                                                <form action="{{ route('frontend.facial-expressions.destroy', $facialExpression->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('facial_expression_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.facial-expressions.massDestroy') }}",
