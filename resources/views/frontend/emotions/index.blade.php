        ...
        <div class="col-md-12">
            @can('emotion_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.emotions.create') }}">
                            ...
                            <tbody>
                                @foreach($emotions as $key => $emotion)
                                    <tr data-entry-id="{{ $emotion->id }}">
                                        ...
                                        <td>
                                            @can('emotion_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.emotions.show', $emotion->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('emotion_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.emotions.edit', $emotion->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('emotion_delete')
                                                <form action="{{ route('frontend.emotions.destroy', $emotion->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('emotion_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.emotions.massDestroy') }}",
