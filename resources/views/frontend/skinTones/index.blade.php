        ...
        <div class="col-md-12">
            @can('skin_tone_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.skin-tones.create') }}">
                            ...
                            <tbody>
                                @foreach($skinTones as $key => $skinTone)
                                    <tr data-entry-id="{{ $skinTone->id }}">
                                        ...
                                        <td>
                                            @can('skin_tone_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.skin-tones.show', $skinTone->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('skin_tone_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.skin-tones.edit', $skinTone->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('skin_tone_delete')
                                                <form action="{{ route('frontend.skin-tones.destroy', $skinTone->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('skin_tone_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.skin-tones.massDestroy') }}",
