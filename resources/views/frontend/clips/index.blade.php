                        ...
                        @can('character_delete')
                            ...
                            <div class="card-body">
                               ...
                               <p class="small muted">{{$clip->created_at->diffForHumans()}} <br><span class="text-muted small"> <i class="fas fa-clock"></i> {{ $clip->duration ?? '00:00:00' }}</span></p>
                                ...
                                @endif
                                    <a href="{{ route('frontend.clips.savelink') }}" id="{{$clip->id}}" rel="{{ $clip->video_path }}" class="btn btn-sm btn-default save_{{ $clip->id }} save" @if($clip->saved==1) disabled="disabled" @endif><i class="fas fa-save  saving_{{ $clip->id }}"></i></a>
