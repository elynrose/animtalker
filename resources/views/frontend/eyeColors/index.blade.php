        ...
        <div class="col-md-12">
            @can('eye_color_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.eye-colors.create') }}">
                            ...
                            <tbody>
                                @foreach($eyeColors as $key => $eyeColor)
                                    <tr data-entry-id="{{ $eyeColor->id }}">
                                        ...
                                        <td>
                                            @can('eye_color_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.eye-colors.show', $eyeColor->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('eye_color_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.eye-colors.edit', $eyeColor->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('eye_color_delete')
                                                <form action="{{ route('frontend.eye-colors.destroy', $eyeColor->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('eye_color_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.eye-colors.massDestroy') }}",
