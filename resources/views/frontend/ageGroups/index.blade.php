        ...
        <div class="col-md-12">
            @can('age_group_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.age-groups.create') }}">
                            ...
                            <tbody>
                                @foreach($ageGroups as $key => $ageGroup)
                                    <tr data-entry-id="{{ $ageGroup->id }}">
                                        ...
                                        <td>
                                            @can('age_group_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.age-groups.show', $ageGroup->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('age_group_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.age-groups.edit', $ageGroup->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('age_group_delete')
                                                <form action="{{ route('frontend.age-groups.destroy', $ageGroup->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('age_group_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.age-groups.massDestroy') }}",
