        ...
        <div class="col-md-12">
            @can('hair_length_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.hair-lengths.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $hairLength->id }}">
                                        ...
                                        <td>
                                            @can('hair_length_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.hair-lengths.show', $hairLength->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('hair_length_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.hair-lengths.edit', $hairLength->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('hair_length_delete')
                                                <form action="{{ route('frontend.hair-lengths.destroy', $hairLength->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hair_length_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.hair-lengths.massDestroy') }}",
