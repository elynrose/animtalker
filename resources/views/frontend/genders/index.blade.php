        ...
        <div class="col-md-12">
            @can('gender_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.genders.create') }}">
                            ...
                            <tbody>
                                @foreach($genders as $key => $gender)
                                    <tr data-entry-id="{{ $gender->id }}">
                                        ...
                                        <td>
                                            @can('gender_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.genders.show', $gender->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('gender_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.genders.edit', $gender->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('gender_delete')
                                                <form action="{{ route('frontend.genders.destroy', $gender->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('gender_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.genders.massDestroy') }}",
