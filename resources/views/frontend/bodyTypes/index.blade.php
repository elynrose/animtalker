        ...
        <div class="col-md-12">
            @can('body_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.body-types.create') }}">
                            ...
                            <tbody>
                                @foreach($bodyTypes as $key => $bodyType)
                                    <tr data-entry-id="{{ $bodyType->id }}">
                                        ...
                                        <td>
                                            @can('body_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.body-types.show', $bodyType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('body_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.body-types.edit', $bodyType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('body_type_delete')
                                                <form action="{{ route('frontend.body-types.destroy', $bodyType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('body_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.body-types.massDestroy') }}",
